...
- recommencer une génération pour gagner de l’Héritage actif, dont l’acquisition ralentit progressivement au fil de la dynastie ;
- conserver cet Héritage pour alimenter le multiplicateur global ou le dépenser dans cinq branches d’améliorations permanentes ;
- confier progressivement la R&D et les optimisations de consoles à des assistants qui préservent toujours 20 % de la trésorerie ;
...
Les gains d’Héritage progressent d’abord logarithmiquement selon les crédits obtenus pendant la génération, puis ralentissent en fonction de tout l’Héritage déjà transmis au cours de la partie. Seul le solde actif alimente le multiplicateur à rendement décroissant : `×(1 + 0,20 × √Héritage actif)`. Dépenser un point réduit donc immédiatement ce multiplicateur, mais l’amélioration achetée persiste entre les générations. Les quatre investissements fondateurs à un point sont volontairement plus puissants que le bonus global sacrifié ; les décisions deviennent ensuite plus spécialisées et plus coûteuses.

Le centre Héritage regroupe le solde actif, le multiplicateur actuel, les Archives acquises, le gain de la prochaine génération et cinq branches : Industrie, Création, Commerce, Technique et Automatisation. Cette dernière peut acheter automatiquement la recherche et les optimisations de parc, puis transmettre le meilleur spécialiste lors d’une nouvelle génération. Chaque assistant reste désactivable et ne dépense jamais les derniers 20 % de la trésorerie.
