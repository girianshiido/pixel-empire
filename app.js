...
const initialState = () => ({
  ...
  nextPitchAt:Date.now()+8000,reputation:0,archivePoints:0,archiveUpgrades:[],combo:{count:0,clicks:0,lastUpdatedAt:0,activeUntil:0,cooldownUntil:0},buyMode:"1",achievements:[],gameCompleted:false,completedAt:0,endingSeen:false,savedAt:Date.now(),
...
function renderPrestige(){
  const gain=prestigeGain(),current=reputationMultiplier(),future=reputationMultiplier({...state,reputation:state.reputation+gain}),spent=archiveUpgrades.filter(item=>hasArchiveUpgrade(item.id)).reduce((sum,item)=>sum+item.cost,0);
  els.prestigeButton.disabled=false;els.prestigeTitle.textContent=gain?`Gagner ${gain} héritage`:"Centre de l’héritage";els.prestigeProgress.textContent=gain?`Production ×${current.toFixed(2).replace(".",",")} → ×${future.toFixed(2).replace(".",",")}`:`Encore ${formatMoney(Math.max(0,1e6-state.runRevenue))} avant la transmission`;els.prestigeReward.textContent=gain?`+${gain} héritage · +${gain} archives · production ×${future.toFixed(2).replace(".",",")}`:"Aucun héritage disponible pour le moment";els.prestigeNote.textContent=`Bonus actuel : production et clic de base ×${current.toFixed(2).replace(".",",")}. Les héritages ne sont jamais dépensés ; les archives financent séparément les spécialisations permanentes.`;els.confirmPrestige.disabled=gain<=0;els.heritageTotal.textContent=format(state.reputation||0);els.heritageMultiplier.textContent=`×${current.toFixed(2).replace(".",",")}`;els.archivePoints.textContent=format(state.archivePoints||0);els.archiveSpent.textContent=`◆ ${format(spent)}`;els.heritageRunRevenue.textContent=formatMoney(state.runRevenue);els.heritageNextGain.textContent=`+${gain} héritage · +${gain} archives`;renderArchiveGrid();if(gain)els.prestigeButton.dataset.ready="true";else delete els.prestigeButton.dataset.ready;
}
