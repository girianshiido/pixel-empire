...
const archiveBranches = [
  {id:"industry",name:"Industrie",icon:"🏭",nodes:[
    {id:"archive_workshop",name:"Atelier transmis",cost:1,kind:"production",value:1.25,description:"Production des consoles ×1,25"},
    {id:"archive_chain",name:"Chaîne patrimoniale",cost:5,kind:"production",value:1.4,description:"Production des consoles ×1,4"},
    {id:"archive_empire",name:"Empire industriel",cost:18,kind:"sales",value:1.6,description:"Capacité de distribution ×1,6"}
  ]},
  {id:"creative",name:"Création",icon:"🎨",nodes:[
    {id:"archive_notebooks",name:"Carnets de maîtres",cost:2,kind:"qualityBonus",value:1,description:"Qualité prévue +1"},
    {id:"archive_school",name:"École du studio",cost:7,kind:"qualityBonus",value:2,description:"Qualité prévue +2"},
    {id:"archive_genres",name:"Culture de genre",cost:20,kind:"genreXp",value:1.5,description:"Expérience des genres ×1,5"}
  ]},
  {id:"commercial",name:"Commerce",icon:"📣",nodes:[
    {id:"archive_network",name:"Réseau d’anciens",cost:2,kind:"price",value:1.15,description:"Prix des copies ×1,15"},
    {id:"archive_catalog",name:"Catalogue héritier",cost:8,kind:"royalty",value:1.35,description:"Royalties ×1,35"},
    {id:"archive_brand",name:"Marque éternelle",cost:22,kind:"marketingDiscount",value:.15,description:"Campagnes marketing 15 % moins chères"}
  ]},
  {id:"technical",name:"Technique",icon:"⚙️",nodes:[
    {id:"archive_shortcuts",name:"Raccourcis transmis",cost:1,kind:"comboDuration",value:3,description:"Focus dure 3 secondes de plus"},
    {id:"archive_prototype",name:"Méthodes de prototypage",cost:6,kind:"projectClick",value:2,description:"Accélération des projets par clic ×2"},
    {id:"archive_pipeline",name:"Pipeline générationnel",cost:20,kind:"projectDuration",value:.85,description:"Développements 15 % plus courts"}
  ]}
];
...
const initialState = () => ({
  ...
  owned:initialOwned(),upgrades:[],genreProgress:initialGenreProgress(),catalog:[],franchises:[],activeProject:null,
  ...
  nextPitchAt:Date.now()+8000,reputation:0,archivePoints:0,archiveUpgrades:[],combo:{count:0,windowStartedAt:0,activeUntil:0,cooldownUntil:0},buyMode:"1",achievements:[],gameCompleted:false,completedAt:0,endingSeen:false,savedAt:Date.now(),
...
const els={
  money:$("#moneyCount"),stock:$("#stockCount"),revenue:$("#revenueRate"),incomeBreakdown:$("#incomeBreakdown"),criticReputation:$("#criticReputation"),reputation:$("#reputationCount"),trendText:$("#trendText"),trendTimer:$("#trendTimer"),trendBanner:$("#trendBanner"),
  ...
  projectCard:$("#projectCard"),projectTimer:$("#projectTimer"),projectTitle:$("#projectTitle"),projectDescription:$("#projectDescription"),projectFill:$("#projectFill"),projectBoosts:$("#projectBoosts"),projectClickBoost:$("#projectClickBoost"),projectRushButton:$("#projectRushButton"),projectButton:$("#projectButton"),
...
function reputationMultiplier(s=state){return 1+.2*Math.sqrt(Math.max(0,s.reputation||0))}
function totalConsoles(s=state){return Object.values(s.owned).reduce((sum,count)=>sum+(Number(count)||0),0)}
function milestoneMultiplier(console,s=state){const owned=s.owned[console.id]||0;return Math.pow(2,MILESTONES.filter(mark=>owned>=mark).length)}
...
function registerComboClick(at=Date.now(),s=state){const combo=s.combo,profile=comboProfile(s);if(combo.activeUntil>at||combo.cooldownUntil>at)return comboMultiplier(s,at);if(combo.count===0||at-combo.windowStartedAt>profile.window){combo.count=1;combo.windowStartedAt=at}else combo.count++;if(combo.count>=profile.required){combo.count=0;combo.windowStartedAt=0;combo.activeUntil=at+profile.duration;combo.cooldownUntil=combo.activeUntil+profile.cooldown;showAchievement(`🔥 Focus ×${profile.multiplier} pendant ${formatDuration(profile.duration/1000)}`);playTone(860,.18)}return comboMultiplier(s,at)}
...
function sequelName(franchise){return `${franchise.name} ${episodeSuffix(franchise.episodes+1)}`}
...
function recruitmentCandidates(s=state){return specialistBranches.map(branch=>specialists.filter(item=>item.branchId===branch.id&&!s.roster.includes(item.id)&&s.lifetimeRevenue>=item.unlockRevenue&&s.studioReputation>=item.unlockReputation&&specialistCareerReady(item,s)).sort((a,b)=>b.skill-a.skill)[0]).filter(Boolean)}
...
function nextMilestone(console,s=state){const owned=s.owned[console.id]||0;return MILESTONES.find(mark=>mark>owned)||null}
...
function prestigeGain(s=state){return s.runRevenue<1e6?0:1+Math.floor(4*Math.log10(s.runRevenue/1e6))}
...
function canStartFinalProject(s=state){return !s.gameCompleted&&!s.activeProject&&finalRequirements(s).every(item=>item.complete)&&s.money>=FINAL_PROJECT_COST}
...
function finishProject(silent=false){
  ...
  state.catalog.unshift(game);updateStudioReputation(review.criticScore);(project.teamIds||[]).forEach(id=>{const record=staffRecord(id);state.staffStats[id]={...record,projects:(record.projects||0)+1}});addCopies(estimate.copies);addGenreXp(genre.id,tier.xp);state.stats.gamesReleased++;updateBestGenreScore(game);state.activeProject=null;state.nextPitchAt=Date.now()+30000;if(state.stats.gamesReleased===1)state.nextEventAt=Math.min(state.nextEventAt,Date.now()+60000);if(project.isFinal){state.gameCompleted=true;state.completedAt=Date.now()}const newAwards=unlockAwards(true);
...
function restoreOffline(){
  ...
  if(elapsed<5)return;
  if(state.activeProject&&state.activeProject.completesAt<=Date.now())finishProject(true);
...
function update(dt){
  ...
  addRevenue(sold*copyPrice()+catalogRoyalties()*dt);
...
function buyArchiveUpgrade(id){
  ...
  state.archivePoints-=upgrade.cost;state.archiveUpgrades.push(id);toast(`${upgrade.name} inscrit dans les archives.`);playTone(760,.16);renderAll();save();
}
...
function openProjectDialog(){
  if(state.activeProject||Date.now()<state.nextPitchAt)return;
...
function renderProjectDialog(){
  ...
  const compatible=genreFranchises(genre.id);els.franchiseChoiceGrid.innerHTML=`<button type="button" class="franchise-choice" data-franchise="" aria-pressed="${!franchise}"><strong>✨ Nouvelle licence</strong><small>Une création originale sans bonus ni fatigue.</small></button>${compatible.map(item=>{const modifiers=franchiseModifiers(item),level=franchiseLevel(item),fatigueLoss=Math.round((1-modifiers.fatigue)*100);return `<button type="button" class="franchise-choice" data-franchise="${item.id}" aria-pressed="${item.id===selectedFranchiseId}"><strong>↻ ${item.name} ${episodeSuffix(item.episodes+1)}</strong><small>Niv. ${level} · audience ${modifiers.audience>=1?"+":""}${Math.round((modifiers.audience-1)*100)} %${fatigueLoss?` · fatigue −${fatigueLoss} %`:""}</small></button>`}).join("")}`;
  els.franchiseHint.textContent=franchise?`Suite : +15 % au budget, +10 % au délai et +${series.quality} en qualité prévue.`:`Le jeu publié deviendra une nouvelle franchise.`;
  ...
  els.startProject.disabled=!tierUnlocked(tier)||state.money<scope.cost||selectedPlatformIds.length<1;els.startProject.textContent=state.money<scope.cost?`Il manque ${formatMoney(scope.cost-state.money)}`:"Lancer le développement";
}
function startProject(){
  const genre=currentGenre(),tier=currentTier(),marketing=currentMarketing(),targets=selectedPlatformIds.filter(id=>availablePlatforms().some(platform=>platform.id===id)).slice(0,3),franchise=currentFranchise(),scope=projectScope(tier,targets,franchise,state.activeTeam,state,marketing),eventEffects={...state.eventEffects};if(state.activeProject||Date.now()<state.nextPitchAt||!genreUnlocked(genre)||!tierUnlocked(tier)||!marketingUnlocked(marketing)||targets.length<1||state.money<scope.cost)return;
  state.money-=scope.cost;const teamIds=[...state.activeTeam],quality=projectQuality(genre,tier,targets,franchise,teamIds,state,eventEffects.nextQuality),startedAt=Date.now(),name=franchise?sequelName(franchise):makeGameName(genre),duration=scope.duration*eventEffects.nextDuration;state.activeProject={genreId:genre.id,tierId:tier.id,platformIds:[...targets],franchiseId:franchise?.id||null,teamIds,invested:scope.cost,name,quality,marketingId:marketing.id,marketingBoost:eventEffects.nextMarketing,marketingExpectationBonus:eventEffects.nextExpectation,startedAt,baseDurationMs:duration*1000,manualAccelerationMs:0,rushesUsed:0,completesAt:startedAt+duration*1000};state.eventEffects.nextQuality=0;state.eventEffects.nextMarketing=1;state.eventEffects.nextDuration=1;state.eventEffects.nextExpectation=0;els.projectDialog.close();toast(`${franchise?"Suite":"Nouvelle licence"} lancée avec ${marketing.name.toLowerCase()}.`);playTone(520,.16);renderAll();save();
}
...
function cancelProject(){
  ...
  state.money+=quote.refund;state.activeProject=null;state.nextPitchAt=Date.now()+8000;toast(`Projet annulé : ${formatMoney(quote.refund)} récupérés.`);playTone(230,.16);renderAll();save();
}

function doPrestige(){
  const gain=prestigeGain();if(gain<=0)return;
  const kept={lifetimeRevenue:state.lifetimeRevenue,lifetimeCopies:state.lifetimeCopies,copiesSold:state.copiesSold,genreProgress:state.genreProgress,bestGenreScores:state.bestGenreScores,awards:state.awards,reputation:state.reputation+gain,archivePoints:(state.archivePoints||0)+gain,archiveUpgrades:state.archiveUpgrades,achievements:state.achievements,gameCompleted:state.gameCompleted,completedAt:state.completedAt,endingSeen:state.endingSeen,settings:state.settings,stats:state.stats};
  state={...initialState(),...kept,nextPitchAt:Date.now()+6000};els.prestigeDialog.close();toast(`Nouvelle génération : +${gain} héritage et +${gain} archives.`);playTone(880,.3);renderAll();save();
}

function renderTop(){
  const salesRevenue=estimatedSalesRevenue(),royalties=catalogRoyalties();els.money.textContent=formatMoney(state.money);els.stock.textContent=format(state.stock);els.revenue.textContent=formatMoney(salesRevenue+royalties);els.incomeBreakdown.textContent=`Ventes ${formatMoney(salesRevenue)}/s · Royalties ${formatMoney(royalties)}/s`;els.criticReputation.textContent=Math.round(state.studioReputation);els.criticReputation.title=reputationLabel(state.studioReputation);els.reputation.textContent=format(state.reputation);
  ...
  renderCombo();renderProjectCard();renderPrestige();
}
function renderCombo(){const now=Date.now(),combo=state.combo,profile=comboProfile();if(combo.windowStartedAt&&now-combo.windowStartedAt>profile.window&&combo.activeUntil<=now){combo.count=0;combo.windowStartedAt=0}const active=combo.activeUntil>now,cooldown=!active&&combo.cooldownUntil>now;els.comboCard.classList.toggle("active",active);els.comboCard.classList.toggle("cooldown",cooldown);if(active){els.comboLabel.textContent="Focus actif";els.comboStatus.textContent=`×${profile.multiplier} · ${formatDuration((combo.activeUntil-now)/1000)}`;els.comboFill.style.width=`${clamp((combo.activeUntil-now)/profile.duration*100,0,100)}%`;els.comboHint.textContent=`Chaque clic fabrique ${profile.multiplier} fois plus de copies.`}else if(cooldown){els.comboLabel.textContent="Récupération";els.comboStatus.textContent=formatDuration((combo.cooldownUntil-now)/1000);els.comboFill.style.width=`${clamp((combo.cooldownUntil-now)/profile.cooldown*100,0,100)}%`;els.comboHint.textContent="La prochaine cadence pourra commencer après la récupération."}else{const left=Math.max(0,profile.window-(now-(combo.windowStartedAt||now)));els.comboLabel.textContent="Cadence";els.comboStatus.textContent=`${combo.count} / ${profile.required}`;els.comboFill.style.width=`${clamp(combo.count/profile.required*100,0,100)}%`;els.comboHint.textContent=combo.count?`${formatDuration(left/1000)} pour atteindre Focus ×${profile.multiplier}.`:`${profile.required} clics en ${(profile.window/1000).toLocaleString("fr-FR")} s déclenchent Focus ×${profile.multiplier}.`}}
...
function renderPrestige(){const gain=prestigeGain(),current=reputationMultiplier(),future=reputationMultiplier({...state,reputation:state.reputation+gain});els.prestigeButton.disabled=gain<=0;els.prestigeTitle.textContent=gain?`Gagner ${gain} héritage`:"Héritage inaccessible";els.prestigeProgress.textContent=gain?`Production ×${current.toFixed(2).replace(".",",")} → ×${future.toFixed(2).replace(".",",")}`:`Encore ${formatMoney(Math.max(0,1e6-state.runRevenue))}`;els.prestigeReward.textContent=`+${gain} héritage · +${gain} archives · production ×${future.toFixed(2).replace(".",",")}`;els.prestigeNote.textContent=`Bonus actuel : production et clic de base ×${current.toFixed(2).replace(".",",")}. Les héritages ne sont jamais dépensés ; les ${format(state.archivePoints||0)} archives disponibles financent séparément les spécialisations permanentes.`;if(gain)els.prestigeButton.dataset.ready="true";else delete els.prestigeButton.dataset.ready}
...
function renderConsoles(){
  ...
  els.consoleList.innerHTML=visible.map(item=>{const owned=state.owned[item.id]||0,isUnlocked=consoleUnlocked(item),quote=purchaseQuote(item),canAfford=isUnlocked&&quote.count>0&&state.money+1e-8>=quote.cost,nextMark=nextMilestone(item),total=owned*item.production*milestoneMultiplier(item)*effect("production",1)*effect("all",1)*archiveEffect("production",1)*reputationMultiplier()*(careerReward("productionMultiplier")?.value||1),affinityNames=(familyAffinities[item.family]||[]).slice(0,2).map(id=>genres.find(genre=>genre.id===id)?.name).filter(Boolean).join(", ");return `<button type="button" class="console-card ${isUnlocked?(canAfford?"":"unaffordable"):"locked"}" data-console="${item.id}" ${canAfford?"":"disabled"}><span class="console-icon">${isUnlocked?item.icon:"?"}</span><span class="console-copy"><span class="console-title"><strong>${isUnlocked?item.name:"Console inconnue"}</strong><span class="generation">${isUnlocked?item.generation:"Verrouillée"}</span></span><p>${isUnlocked?item.description:`Atteins ${formatMoney(item.baseCost/12)} de revenus pour la découvrir.`}</p><span class="console-meta"><span>Possédées <b>${owned}</b></span><span>Production <b>${format(total)}/s</b></span>${isUnlocked&&affinityNames?`<span>Idéale pour <b>${affinityNames}</b></span>`:""}${nextMark?`<span>Prochain palier <b>${nextMark} · ×2</b></span>`:""}</span></span><span class="console-buy"><strong>${quote.count>0?`+${quote.count}`:"+1"}</strong><span>${formatMoney(quote.cost)}</span></span></button>`}).join("");
}
function renderUpgrades(){
  const available=availableUpgrades(),future=upgrades.filter(item=>!hasUpgrade(item.id)&&state.lifetimeRevenue<item.unlock).sort((a,b)=>a.unlock-b.unlock);els.upgradeBadge.textContent=available.length;els.upgradeList.innerHTML=available.map(item=>`<article class="upgrade-card"><span>${item.icon}</span><div><h2>${item.name}</h2><p>${item.description}</p><button type="button" data-upgrade="${item.id}" ${state.money<item.cost?"disabled":""}>${formatMoney(item.cost)}</button></div></article>`).join("");els.upgradeEmpty.hidden=available.length>0;if(!available.length)els.upgradeEmpty.innerHTML=future.length?`<span>◌</span><strong>Prochaine recherche à ${formatMoney(future[0].unlock)}</strong><p>${future[0].name} apparaîtra avec les revenus cumulés du studio.</p>`:`<span>✓</span><strong>Toutes les recherches sont installées</strong><p>Le studio a atteint le sommet de son équipement.</p>`;
  els.archivePoints.textContent=format(state.archivePoints||0);els.archiveGrid.innerHTML=archiveBranches.map(branch=>`<section class="archive-branch"><header><span>${branch.icon}</span><strong>${branch.name}</strong></header>${branch.nodes.map(raw=>{const node=archiveUpgrades.find(item=>item.id===raw.id),bought=hasArchiveUpgrade(node.id),requires=node.requires?hasArchiveUpgrade(node.requires):true,canBuy=!bought&&requires&&state.archivePoints>=node.cost;return `<article class="archive-node ${bought?"bought":requires?"available":"locked"}"><span>${bought?"✓":node.index+1}</span><p><strong>${node.name}</strong><small>${node.description}</small></p><button type="button" data-archive-upgrade="${node.id}" ${canBuy?"":"disabled"}>${bought?"Acquis":requires?`${node.cost} ◆`:"Verrouillé"}</button></article>`}).join("")}</section>`).join("");
}
...
els.prestigeButton.addEventListener("click",()=>{if(prestigeGain()>0){renderPrestige();els.prestigeDialog.showModal()}});els.confirmPrestige.addEventListener("click",doPrestige);
