...
const initialState = () => ({
  ...
  nextPitchAt:Date.now()+8000,reputation:0,buyMode:"1",achievements:[],gameCompleted:false,completedAt:0,endingSeen:false,savedAt:Date.now(),
  settings:{effects:true,sound:true},stats:{clicks:0,consolesBought:0,gamesReleased:0,playSeconds:0,bestRevenue:0}
});
...
const els={
  ...
  developer:$("#developerButton"),perClick:$("#perClick"),manualShare:$("#manualShare"),salesCapacity:$("#salesCapacity"),salesFill:$("#salesFill"),copyPrice:$("#copyPrice"),studioStatus:$("#studioStatus"),
  projectCard:$("#projectCard"),projectTimer:$("#projectTimer"),projectTitle:$("#projectTitle"),projectDescription:$("#projectDescription"),projectFill:$("#projectFill"),projectButton:$("#projectButton"),
...
function formatDuration(seconds){if(seconds<60)return `${Math.ceil(seconds)} s`;const minutes=Math.floor(seconds/60),rest=Math.ceil(seconds%60);return rest?`${minutes} min ${rest} s`:`${minutes} min`}
...
function marketingCost(plan,s=state){return plan.cost*(1-(careerReward("marketingDiscount",s)?.value||0))}
...
function production(s=state){return baseProduction(s)*effect("production",1,(a,b)=>a*b,s)*effect("all",1,(a,b)=>a*b,s)*reputationMultiplier(s)*(careerReward("productionMultiplier",s)?.value||1)}
function clickValue(s=state){return effect("click",1,(a,b)=>a*b,s)*reputationMultiplier(s)+effect("clickPps",0,(a,b)=>a+b,s)*production(s)}
function copyPrice(s=state){return 3*effect("price",1,(a,b)=>a*b,s)*effect("all",1,(a,b)=>a*b,s)}
function salesCapacity(s=state){return Math.max(3,production(s)*1.15+s.catalog.length*.5)*effect("sales",1,(a,b)=>a*b,s)*effect("all",1,(a,b)=>a*b,s)}
...
function gameRoyalty(game,s=state){const franchise=franchiseForGame(game,s),catalogBonus=franchise?1+(franchiseLevel(franchise)-1)*.03:1;return game.baseRoyalty*genreLevelMultiplier(game.genreId,s)*demandMultiplier(game.genreId)*catalogBonus*effect("royalty",1,(a,b)=>a*b,s)*effect("all",1,(a,b)=>a*b,s)*(careerReward("royaltyMultiplier",s)?.value||1)*activeGlobalRoyalty(s)}
...
function addGenreXp(id,amount){
  const progress=genreProgress(id);progress.xp+=amount;
  let leveled=false;
...
function renderConsoles(){
  ...
  els.consoleList.innerHTML=visible.map(item=>{const owned=state.owned[item.id]||0,isUnlocked=consoleUnlocked(item),quote=purchaseQuote(item),canAfford=isUnlocked&&quote.count>0&&state.money+1e-8>=quote.cost,nextMark=nextMilestone(item),total=owned*item.production*milestoneMultiplier(item)*effect("production",1)*reputationMultiplier(),affinityNames=(familyAffinities[item.family]||[]).slice(0,2).map(id=>genres.find(genre=>genre.id===id)?.name).filter(Boolean).join(", ");return `<button type="button" class="console-card ${isUnlocked?(canAfford?"":"unaffordable"):"locked"}" data-console="${item.id}" ${canAfford?"":"disabled"}><span class="console-icon">${isUnlocked?item.icon:"?"}</span><span class="console-copy"><span class="console-title"><strong>${isUnlocked?item.name:"Console inconnue"}</strong><span class="generation">${isUnlocked?item.generation:"Verrouillée"}</span></span><p>${isUnlocked?item.description:`Atteins ${formatMoney(item.baseCost/12)} de revenus pour la découvrir.`}</p><span class="console-meta"><span>Possédées <b>${owned}</b></span><span>Production <b>${format(total)}/s</b></span>${isUnlocked&&affinityNames?`<span>Idéale pour <b>${affinityNames}</b></span>`:""}${nextMark?`<span>Prochain palier <b>${nextMark} · ×2</b></span>`:""}</span></span><span class="console-buy"><strong>${quote.count>0?`+${quote.count}`:"+1"}</strong><span>${formatMoney(quote.cost)}</span></span></button>`}).join("");
}
function renderUpgrades(){const available=availableUpgrades();els.upgradeBadge.textContent=available.length;els.upgradeList.innerHTML=available.map(item=>`<article class="upgrade-card"><span>${item.icon}</span><div><h2>${item.name}</h2><p>${item.description}</p><button type="button" data-upgrade="${item.id}" ${state.money<item.cost?"disabled":""}>${formatMoney(item.cost)}</button></div></article>`).join("");els.upgradeEmpty.hidden=available.length>0}
...
els.developer.addEventListener("click",()=>{const value=clickValue();addCopies(value);state.stats.clicks++;els.developer.classList.add("clicked");setTimeout(()=>els.developer.classList.remove("clicked"),100);playTone(360+Math.min(300,state.stats.clicks%12*18),.05);renderTop()});
